"""Full translation and harmonization pipeline for Claude Desktop Chinese localization."""
...
print(f"Total keys in dictionary: {len(data)}")
print(f"Existing CJK keys: {sum(1 for v in data.values() if has_cjk(v))}")
print(f"Existing non-CJK keys: {sum(1 for v in data.values() if not has_cjk(v))}")
